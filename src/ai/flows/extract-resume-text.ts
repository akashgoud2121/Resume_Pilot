...
/**
 * @fileOverview A resume text extraction AI agent.
 *
...
const prompt = ai.definePrompt({
  ...
  prompt: `You are an expert text extractor. Extract all the text from the provided resume document.

Preserve the formatting and line breaks as much as possible.
...
const extractResumeTextFlow = ai.defineFlow(
  ...
  async input => {
    ...
    if (!output?.resumeText) {
      throw new Error('AI failed to extract resume text. The output was empty.');
    }
    return output;
  }
);
