...
export function MinimalTemplate({ data }: TemplateProps) {
  return (
    <div className="p-12 bg-white text-gray-700 font-light" style={{ fontFamily: "'Helvetica Neue', Helvetica, Arial, sans-serif" }}>
      {/* Header */}
      <header className="text-center mb-10">
        <h1 className="text-4xl font-normal text-gray-900 tracking-wider">{data.name}</h1>
        <div className="flex justify-center items-center gap-x-6 gap-y-1 mt-3 text-xs text-gray-500">
          {data.email && <a href={`mailto:${data.email}`} className="hover:text-black">{data.email}</a>}
            ...
            {data.experience.map((exp, index) => (
              <div key={index}>
                <div className="flex justify-between items-baseline">
                  <h3 className="font-medium text-gray-900">{exp.title}, <span className="text-gray-600">{exp.company}</span></h3>
